...
def train(args, model, train_loader, dev_loader, logger):
    ...
    for epoch in range(args.epoch_num):
        ...
        for i, (claim_ids, claim_msks, ce_ids, ce_msks, labels) in tqdm(enumerate(train_loader),
            ...
            loss_c = F.cross_entropy(out_c, labels.long(), weight=torch.Tensor([4., 1., 2.]).cuda())
            loss_ce = F.cross_entropy(out_ce, labels.long(), weight=torch.Tensor([4., 1., 2.]).cuda())
...
def main(args):
    ...
    if args.mode == "train":
        ...
        log_path = log_path + "{}_constraint_{}_claim_{}_scaled_{}.log".format(args.num_classes, args.constraint_loss_weight, args.claim_loss_weight, args.scaled_rate)
    elif args.mode == "test":
...
if __name__ == '__main__':
    ...
    # parser.add_argument("--saved_model_path", type=str, default="./save_models/[DATASET]/three_unbiased_412_[constraint]_[claim]_[scaled].pth")
    parser.add_argument("--saved_model_path", type=str, default="./save_models/[DATASET]/politihop_3.pth")

    parser.add_argument("--test_results", type=str, default="./para_results/politihop_3.txt")
