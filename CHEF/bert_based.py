...
parser.add_argument('--cuda', type=str, default="6",help='appoint GPU devices')
...
def main(argv = None):
    datalist = json.load(open('/data/yangjun/fact/debias/data/raw/dev.json', 'r', encoding='utf-8')) \
        + json.load(open('/data/yangjun/fact/debias/data/raw/train.json', 'r', encoding='utf-8'))
    ...
    for data in datalist:
        claim = data['claim']
        evidences = data['gold evidence']
        ce_pair = "[CLS] {} [SEP]".format(claim)
        for evidence in evidences:
            ce_pair += " {} [SEP]".format(evidence)
        sentence.append(ce_pair)
...
def train_and_save_model(model, train_dataloader, val_dataloader, bert_type='bert', model_save_dir='./saved_models/'):
