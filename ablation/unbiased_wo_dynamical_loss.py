...
def train(args, model, train_loader, dev_loader, logger):
    ...
    for epoch in range(args.epoch_num):
        ...
        for i, (claim_ids, claim_msks, evidence_ids, evidence_msks, ce_ids, ce_msks, labels) in tqdm(enumerate(train_loader),
            ...
            loss = loss_ce.sum() + args.claim_loss_weight * loss_c.sum() + args.evidence_loss_weight * loss_e.sum()
            loss.backward()
...
def main(args):
    ...
    if args.mode == "train":
        log_path = log_path + "unbiased_wo_dynamical_loss_cc_{}_ce_{}_claim_{}_evidence_{}.log".format(args.constraint_claim_loss_weight, args.constraint_evidence_loss_weight, args.claim_loss_weight, args.evidence_loss_weight)
    elif args.mode == "test":
...
if __name__ == '__main__':
    ...
    parser.add_argument("--saved_model_path", type=str, default="./ablation/models/unbiased_wo_dynamical_loss_[CC]_[CE]_[claim]_[evidence]_[scaled].pth")
