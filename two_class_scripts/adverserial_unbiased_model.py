...
def train(args, claim_only_model, evidence_only_model, model, train_loader, dev_loader, logger):
    ...
    for epoch in range(args.epoch_num):
        ...
        for _, (claim_ids, claim_msks, evidence_ids, evidence_msks, ce_ids, ce_msks, labels) in tqdm(enumerate(train_loader),
            ...
            loss_overall = loss_ce.sum() - args.claim_loss_weight * loss_c.sum() - args.evidence_loss_weight * loss_e.sum()
...
def main(args):
    # init logger
    if args.mode == "train":
        log_path = args.log_path + "{}_class_CHEF_adverserial_unbiased_claim_{}_evidence_{}.log".format(args.num_classes, args.claim_loss_weight, args.evidence_loss_weight)
    elif args.mode == "test":
...
if __name__ == '__main__':
    ...
    parser.add_argument("--log_path", type=str, default='./logs/para/')
    parser.add_argument("--data_path", type=str, default="./data/processed/[DATA]_2.json")
    parser.add_argument("--saved_model_path", type=str, default="./models/para/two_CHEF_adverserial_unbiased_[MODEL]_[constraint]_[claim].pth")
    ...
    parser.add_argument("--claim_loss_weight", type=float, default=0.001)
    parser.add_argument("--evidence_loss_weight", type=float, default=0.001)
